...
import { AnimationAction, AnimationMixer, AnimationClip, Box3, LoopOnce, LoopRepeat, Object3D, Vector3 } from "three";
...
const clickedQuery = enterQuery(
  defineQuery([AnimationOnClick, NetworkedAnimationOnClick, SingleActionButton, Interacted])
);
...
function stopClipsForEntity(
  ...
  const myClips = ctx.root.animations.filter(clip => clip.tracks.some(track => uuids.has(track.name.split(".")[0])));
...
function playClips(
  ...
  let clips = root.animations.filter(clip => clip.tracks.some(track => uuids.has(track.name.split(".")[0])));
